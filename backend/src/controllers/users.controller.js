...
export async function uploadAvatar(req, res, next) {
  try {
    ...
    if (!IMAGE_MIME.has(mime))
      throw badRequest("Chỉ chấp nhận ảnh JPEG, PNG, WebP hoặc GIF");
    ...
    const safeName =
      `${Date.now()}_${Math.random().toString(16).slice(2)}_${file.originalname}`.replaceAll(
        "..",
        "",
      );
...
export async function changePassword(req, res, next) {
  try {
    ...
    const r = await query("SELECT password FROM users WHERE id = $1 LIMIT 1", [
      id,
    ]);
    if (!r.rows.length) throw notFound("Không tìm thấy người dùng");
